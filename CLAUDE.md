...
- **`scripts/setup-service.ps1` and `scripts/update.ps1` must agree on `-ServiceName`** — `setup-service.ps1` creates the systemd unit/Windows Service under whatever name you pass it (default `"nwsalertbot"`); `update.ps1`'s restart-detection looks for a service by that same name (its own default is also `"nwsalertbot"`, but only matches the *first* instance). Running more than one instance on a machine (see README "Running as a Service") means passing a distinct, matching `-ServiceName` to both scripts for each instance — there's no config-driven way to infer this automatically.
