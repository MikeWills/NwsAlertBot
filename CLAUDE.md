...
- **Auto-Update has known unattended-operation gaps, documented in README "Known Limitations" (under Auto-Update) — read that before touching `UpdateCheckService`/`update.ps1`/`setup-service.ps1`.** Summary: Linux restart needs passwordless `sudo` (undocumented requirement, easy to hit silently); the Windows-Service self-stop-during-update path is theoretically safe but has never been exercised against a real service (possible race between SCM failure-recovery and the file swap); there's no rollback if a bad release is applied, no logged confirmation that an update succeeded, and no checksum/signature verification on downloaded releases. None of these are regressions — they're accepted-for-now gaps in a v1, called out explicitly so they don't get rediscovered as surprises.
