...
- **`scripts/setup-service.ps1`'s `-ServiceName` and `UpdateSettings.ServiceName` must agree** — `setup-service.ps1` creates the systemd unit/Windows Service under whatever name you pass it (default `"nwsalertbot"`); `UpdateCheckService.LaunchUpdater` passes `Update.ServiceName` (same default) to `update.ps1` as its own `-ServiceName`, so its restart-detection looks for a service by that name. Running more than one instance on a machine (see README "Running as a Service") means setting a distinct, matching name in both places per instance — `setup-service.ps1 -ServiceName X` and that instance's `appsettings.json` `Update.ServiceName: "X"`. If you add a new call site that launches `update.ps1` (manually or otherwise), don't forget `-ServiceName` — it's easy to drop since `update.ps1`'s own default silently "works" (just against the wrong/first instance) if omitted.
