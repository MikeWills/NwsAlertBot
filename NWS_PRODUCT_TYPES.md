# NWS Text Product Type Reference

Complete list of NWS text product codes available from the products API
(`https://api.weather.gov/products/types`), sorted alphabetically by code.

This is a separate feed from the CAP alerts feed (`/alerts/active`) used for VTEC-based
warnings/watches/advisories and SPS — see [NWS_EVENT_TYPES.md](NWS_EVENT_TYPES.md) for that.
This products feed is fetched per-office (`/products/types/{code}/locations/{wfo}`) or
unfiltered by type (`/products?type={code}`), and returns raw teletype-formatted text,
not structured alert data.

Source query: `GET https://api.weather.gov/products/types` (338 codes as of 2026-07-01).

### Candidates for future support

Not implemented — flagged here as plausible future additions, same pattern as `HwoService`:

- **AFD** (Area Forecast Discussion) — long-form meteorologist reasoning/discussion, similar
  in spirit to HWO (personal-use text, not suited to short-form platforms).
- **RFW** (Red Flag Warning) — likely redundant: fire weather warnings already arrive through
  the VTEC/CAP alerts feed (`/alerts/active`) that the main pipeline polls, so this text-product
  version would probably duplicate alerts already being posted.
- **LSR** (Local Storm Report) — real-time spotter/storm reports; could be useful for
  "eyes on the ground" info but has no polygon and would need its own dedup/filtering logic.
...
| AFD | Area Forecast Discussion | Candidate (see above) |
...
| HMD | National Hydromet Discussion |  |
| HML | AHPS XML |  |
...
| HWO | Hazardous Weather Outlook | Hazardous Weather Outlook — HwoService |
...
| LSR | Local Storm Report | Candidate (see above) |
| LTG | Lightning Data |  |
...
| RFI | RFI Observation |  |
| RFR | Route Forecast |  |
| RFW | Red Flag Warning | Candidate (see above) |
...
| SWO | Severe Storm Outlook Narrative (AC) | SPC Mesoscale Discussion (MCD) — SpcMcdService (filters this feed for 'SWOMCD' header) |
